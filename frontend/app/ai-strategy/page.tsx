...
export default function AIStrategyPage() {
  ...
  // Helper function to determine if pit stop is needed based on telemetry
  const isPitStopNeeded = () => {
    if (!backendState || !predictions) return false;

    const fuelLow = backendState.fuel < 10; // Less than 10L
    const tiresWorn = (backendState.tire_age || 0) > 15; // More than 15 laps
    const driverInconsistent =
      predictions.driver_consistency === 0;

    // If any critical condition is met, recommend pit stop
    return (
      fuelLow ||
      tiresWorn ||
      (driverInconsistent && (backendState.tire_age || 0) > 10)
    );
  };

  // Helper function to adjust lap time prediction based on telemetry
  const getAdjustedLapTime = (predictedTime: number) => {
    if (!backendState) return predictedTime;

    const avgLapTime = 105; // Average baseline lap time in seconds
    const maxDeviation = 5; // Maximum allowed deviation from baseline

    // If prediction deviates too much from realistic times, blend it with calculation
    if (Math.abs(predictedTime - avgLapTime) > maxDeviation) {
      // Calculate estimated lap time based on current conditions
      let calculatedTime = avgLapTime;

      // Adjust for tire age (0.05s per lap worn)
      calculatedTime += (backendState.tire_age || 0) * 0.05;

      // Adjust for fuel load (lighter = faster)
      const fuelEffect = (50 - (backendState.fuel || 25)) * 0.02;
      calculatedTime -= fuelEffect;

      // Adjust for weather
      if (rawTelemetry.RAINFALL_PERCENTAGE > 20) {
        calculatedTime += rawTelemetry.RAINFALL_PERCENTAGE * 0.05;
      }

      // Blend 70% calculated + 30% predicted
      return calculatedTime * 0.7 + predictedTime * 0.3;
    }
  ...
  useEffect(() => {
    ...
    const fetchPredictions = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/predict?user_id=${userId}`);
  ...
  if (authLoading) {
    return <LoadingScreen message="Loading AI Strategy..." />;
  }
                  ...
                  <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-3 hover:border-orange-600/30 transition-colors">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-lg md:text-xl font-bold truncate text-orange-500">
                          {rawTelemetry.TRACK_TEMP
                            ? Math.round(rawTelemetry.TRACK_TEMP)
                            : 0}
                          °C
                        </div>
                      </div>
                  ...
                  <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-3 hover:border-orange-600/30 transition-colors">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-lg md:text-xl font-bold truncate text-orange-500">
                          {rawTelemetry.RAINFALL_PERCENTAGE
                            ? Math.round(rawTelemetry.RAINFALL_PERCENTAGE)
                            : 0}
                          %
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div className="text-xs text-green-500 font-bold uppercase tracking-wide">
                        Tire Strategy
                      </div>
                    </div>
                    <p className="text-sm text-zinc-400 leading-relaxed">
                      Tire degradation optimal. Continue current pace. Pit
                      window opens at lap {currentLap + 8}-{currentLap + 12}.
                    </p>
                  </div>
                  ...
                  <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div className="text-xs text-orange-500 font-bold uppercase tracking-wide">
                        Fuel Management
                      </div>
                    </div>
                    <p className="text-sm text-zinc-400 leading-relaxed">
                      Reduce 2% consumption to extend pit window by 3 laps.
                      Current rate:{" "}
                      {predictions?.fuel_consumption.toFixed(2) || "2.10"}L/lap.
                    </p>
                  </div>
                  ...
                  <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                    ...
                    <p className="text-sm text-zinc-400 leading-relaxed">
                      Hold position. Overtake window in sector 2 after lap{" "}
                      {currentLap + 5}. Gap management critical.
                    </p>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="border border-zinc-800/50 rounded-lg p-3 bg-zinc-900/30">
                    <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide font-semibold">
                      Track
                    </p>
                  ...
                  <div className="border border-zinc-800/50 rounded-lg p-3 bg-zinc-900/30">
                    ...
                    <p className="text-xl font-bold text-orange-500">
                      {currentLap}
                      <span className="text-zinc-500 text-sm">/35</span>
                    </p>
                  </div>
                  <div className="border border-zinc-800/50 rounded-lg p-3 bg-zinc-900/30">
                    <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide font-semibold">
                    ...
                    <>
                      <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 mb-4 hover:border-orange-600/30 transition-colors">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm text-zinc-500 uppercase tracking-wide font-semibold">
                              Next Lap Forecast
                            </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                        <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                          ...
                          <div className="text-xs text-zinc-500 mt-2 pt-2 border-t border-zinc-800/50 font-medium">
                            Model: Regression | MAE: 0.049s | R²: 1.000
                          </div>
                        </div>
                        ...
                        <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                          ...
                          <div className="text-xs text-zinc-500 mt-2 pt-2 border-t border-zinc-800/50 font-medium">
                            Model: Regression | MAE: 0.119L | R²: 0.973
                          </div>
                        </div>
                        ...
                        <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-4 hover:border-orange-600/30 transition-colors">
                          ...
                          <div className="text-xs text-zinc-500 mt-2 pt-2 border-t border-zinc-800/50 font-medium">
                            Model: Regression | MAE: 1.342s | R²: 0.132
                          </div>
                        </div>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-2xl font-bold text-green-500">
                                {typeof predictions.driver_consistency ===
                                "number"
                                  ? predictions.driver_consistency === 1
                                    ? "High"
                                    : "Low"
                                  : predictions.driver_consistency || "High"}
                              </div>
                            </div>
                          </div>
                          <div className="text-xs text-zinc-500 mt-2 pt-2 border-t border-zinc-800/50 font-medium">
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-2xl font-bold text-orange-500">
                                ...
                                {predictions.weather_impact.toFixed(3)}s
                              </div>
                            </div>
                          </div>
                          <div className="text-xs text-zinc-500 mt-2 pt-2 border-t border-zinc-800/50 font-medium">
                            Model: Regression | MAE: 0.009s | R²: 0.999
                          </div>
                        </div>
                              ...
                              <div className="text-2xl font-bold text-orange-500">
                                {(() => {
                                  const modelPrediction =
                                    typeof predictions.weather_pit_strategy ===
                                    "number"
                                      ? predictions.weather_pit_strategy === 1
                                        ? "High"
                                        : "Low"
                                      : predictions.weather_pit_strategy;

                                  // Override if telemetry indicates pit stop needed
                                  if (isPitStopNeeded()) {
                                    return "High";
                                  }
                      ...
                      <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-6 hover:border-orange-600/30 transition-colors mb-6">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <span className="text-xs px-2 py-1 bg-orange-500/20 text-orange-500 rounded-full font-bold ml-auto">
                            TARGET
                          </span>
                        </div>
                        <div className="grid grid-cols-3 gap-6">
                          <div className="text-center">
                            ...
                            <div className="border border-zinc-800/50 rounded-lg p-4 bg-zinc-900/30">
                              <p className="text-3xl font-bold text-orange-500">
                                {predictions.optimal_sector.s1.toFixed(3)}
                          ...
                          <div className="text-center">
                            ...
                            <div className="border border-zinc-800/50 rounded-lg p-4 bg-zinc-900/30">
                              <p className="text-3xl font-bold text-orange-500">
                                {predictions.optimal_sector.s2.toFixed(3)}
                                <span className="text-sm text-zinc-500 ml-1">
                                  s
                                </span>
                              </p>
                          ...
                          <div className="text-center">
                            ...
                            <div className="border border-zinc-800/50 rounded-lg p-4 bg-zinc-900/30">
                              <p className="text-3xl font-bold text-orange-500">
                                {predictions.optimal_sector.s3.toFixed(3)}
                                <span className="text-sm text-zinc-500 ml-1">
                                  s
                                </span>
                              </p>
                          ...
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-zinc-500">
                              ...
                              <span className="text-orange-500 font-bold">
                                {(
                                  predictions.optimal_sector.s1 +
                                  predictions.optimal_sector.s2 +
                                  predictions.optimal_sector.s3
                                ).toFixed(3)}
                                s
                              </span>
                            </span>
                            <span className="text-zinc-500">
                              Model: Regression | R²: 0.997
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="border border-zinc-800 rounded-lg bg-zinc-950 p-6 hover:border-orange-600/30 transition-colors">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                          {[1, 2, 3, 4, 5].map((offset) => {
                            ...
                            const baseLapTime = getAdjustedLapTime(
                              predictions.lap_time
                            );

                            // Calculate projected time with tire degradation
                            const tireDegradation = offset * 0.05; // 0.05s per lap
                            const projectedTime =
                              baseLapTime +
                              tireDegradation +
                              (Math.random() * 0.3 - 0.15);

                            const projectedFuel = backendState?.fuel
                              ? backendState.fuel -
                                predictions.fuel_consumption * offset
                              : 0;
                              ...
                              >
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-sm font-bold text-orange-500">
                                      {(backendState?.tire_age || 0) + offset}L
                                    </span>
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-zinc-800/50">
                          <div className="flex items-center justify-between text-xs text-zinc-500">
                            <span>
                              📊 Projections based on current ML predictions and
                              historical patterns
                            </span>
                            <span className="text-orange-500 font-bold">
                              ±0.25s variance

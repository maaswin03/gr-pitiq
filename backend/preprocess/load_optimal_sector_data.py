"""
Data Loading Pipeline for Optimal Sector Time Predictor
...
class OptimalSectorDataLoader:
    """Data loader for optimal sector prediction"""
    ...
    def load_all_data(self) -> pd.DataFrame:
        ...
        """
        print("=" * 80)
        print("🏎️  Optimal Sector Predictor - Data Loading Pipeline")
        print("=" * 80)
    ...
    def _load_endurance_data(self, track: str, race: int) -> pd.DataFrame:
        ...
        # Parse lap time and sector times from MM:SS.mmm to seconds
        ...
        # Remove invalid laps (keep only laps with valid sector times for sector prediction)
        df = df[
            (df['LAP_TIME_SECONDS'] > 0) &
            (df['LAP_TIME_SECONDS'] < 300) &
            (df['S1_SECONDS'] > 0) &  # Must have valid S1
            (df['S2_SECONDS'] > 0) &  # Must have valid S2
            (df['S3_SECONDS'] > 0) &  # Must have valid S3
    ...
    def _load_weather_data(self, track: str, race: int) -> pd.DataFrame:
        ...
        weather_summary = {
            ...
            'TRACK_TEMP': df['TRACK_TEMP'].fillna(df['AIR_TEMP'] * 1.2).mean(),
...
if __name__ == "__main__":
    ...
    print("\n📈 Sample Sector Times:")
    print(df[['TRACK', 'RACE', 'NUMBER', 'LAP_NUMBER', 'S1_SECONDS', 'S2_SECONDS', 'S3_SECONDS']].head(10))

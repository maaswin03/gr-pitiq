"""
Fuel Consumption Predictor - Testing & Validation
...
class FuelConsumptionTester:
    """Test fuel consumption predictor"""
    ...
    def load_model(self):
        ...
        model_file = self.model_dir / "fuel_consumption_predictor.pkl"
        metadata_file = self.model_dir / "metadata" / "fuel_consumption_predictor.json"
    ...
    def load_data(self):
        ...
        print(f"   Features: {len(self.feature_names)}")
        print(f"   Target Range: {y.min():.2f} - {y.max():.2f} L/lap")
        print(f"   Target Mean: {y.mean():.2f} L/lap")
    ...
    def predict(self, X):
        ...
        print(f"   Predicted Range: {y_pred.min():.2f} - {y_pred.max():.2f} L/lap")
        print(f"   Predicted Mean: {y_pred.mean():.2f} L/lap")
    ...
    def evaluate_overall(self, y_true, y_pred):
        ...
        print(f"   RMSE:      {rmse:.4f} L/lap")
        print(f"   MAE:       {mae:.4f} L/lap")
        print(f"   MSE:       {mse:.4f}")
        
        # Error analysis
        errors = y_pred - y_true
        print(f"\n📊 Error Analysis:")
        print(f"   Mean Error:     {errors.mean():.4f} L/lap")
        print(f"   Std Error:      {errors.std():.4f} L/lap")
        print(f"   Max Error:      {errors.abs().max():.4f} L/lap")
        print(f"   Median Error:   {np.median(errors):.4f} L/lap")
    ...
    def show_sample_predictions(self, df, y_true, y_pred, n_samples=10):
        ...
        print(f"\n🎲 Random {len(sample_indices)} Predictions:\n")
        ...
        for i, idx in enumerate(sample_indices):
            ...
            lap_time = df.iloc[idx].get('LAP_TIME', 0)
            
            print(f"   {i+1:2d}. Track: {track:12s} | Lap: {lap_num:3.0f} | Lap Time: {lap_time:6.2f}s")
            print(f"       Actual: {actual:.3f} L → Predicted: {predicted:.3f} L")
            print(f"       Error: {error:+.3f} L ({pct_error:+.1f}%)")
    ...
    def save_results(self, df, y_pred, metrics):
        ...
        results_df = df.copy()
        results_df['PREDICTED_FUEL'] = y_pred
        results_df['PREDICTION_ERROR'] = y_pred - results_df['FUEL_CONSUMPTION']
        results_df['PERCENTAGE_ERROR'] = (results_df['PREDICTION_ERROR'] / results_df['FUEL_CONSUMPTION'] * 100).abs()
        ...
        with open(metrics_file, 'w') as f:
            f.write("FUEL CONSUMPTION PREDICTOR - TEST METRICS\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"R² Score:  {metrics['r2']:.4f}\n")
            f.write(f"RMSE:      {metrics['rmse']:.4f} L/lap\n")
            f.write(f"MAE:       {metrics['mae']:.4f} L/lap\n")
...
def main():
    print("=" * 80)
    print("⛽ FUEL CONSUMPTION PREDICTOR - TESTING PIPELINE")
    print("=" * 80)
    
    try:
        # Initialize tester
        tester = FuelConsumptionTester()
        ...
        print(f"   MAE:      {metrics['mae']:.4f} L/lap")
        print(f"   RMSE:     {metrics['rmse']:.4f} L/lap")

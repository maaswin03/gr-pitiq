"""
Professional Ensemble Weather Impact Prediction Model
LightGBM + XGBoost + CatBoost with Optuna Hyperparameter Tuning
Predicts weather impact delta on lap time (WEATHER_DELTA)
"""
...
class WeatherImpactPredictor:
    """Expert-level ensemble weather impact prediction model"""
    
    def __init__(self, output_dir: str = None, model_dir: str = None):
        # Get root directory (3 levels up from backend/train/)
        root_dir = Path(__file__).parent.parent.parent
        
        # Set output directory for weather_impact_predictor
        if output_dir is None:
            self.output_dir = root_dir / "outputs" / "weather_impact_predictor"
        else:
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple:
        ...
        # Define target - Single output: WEATHER_DELTA (weather impact on lap time)
        ...
        exclude_cols = [
            ...
            'CAR_NUMBER', 'DRIVER', 'RACE_NUMBER',  # Identifiers
        ...
        y = df_clean[target].copy()  # Single output: WEATHER_DELTA
        track_labels = df_clean['TRACK'].copy() if 'TRACK' in df_clean.columns else None
        ...
        print(f"✅ Target range: {y.min():.4f}s - {y.max():.4f}s (mean: {y.mean():.4f}s)")
        
        # Print track distribution before split (if available)
        ...
        # Split data: 70% train, 15% validation, 15% test WITH STRATIFICATION (if track available)
        if track_labels is not None:
            ...
            X_train, X_val, y_train, y_val, track_train, track_val = train_test_split(
                X_temp, y_temp, track_temp, test_size=0.176, random_state=42, stratify=track_temp
            )
        else:
            ...
            track_train = track_val = track_test = None
        
        print(f"\n✅ Train: {len(X_train)} | Val: {len(X_val)} | Test: {len(X_test)}")
        
        if track_train is not None:
    ...
    def evaluate_models(self, X_test, y_test, track_test=None, X_val=None, y_val=None):
        ...
        Single-output evaluation for WEATHER_DELTA
        """
        print("\n" + "=" * 80)
        print("📊 Model Evaluation (Weather Impact: WEATHER_DELTA)")
        print("=" * 80)
    ...
    def save_model(self):
        ...
        # Save ensemble model to backend/model/weather_impact_predictor.pkl
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("🏎️  GR PitIQ - ENHANCED Weather Impact Prediction Model")
    print("=" * 80)
    
    # Get root directory (3 levels up from backend/train/)
    root_dir = Path(__file__).parent.parent.parent
    
    # Load ENHANCED dataset from outputs/weather_impact_predictor/
    print("\n📂 Loading dataset...")
    data_path = root_dir / "outputs" / "weather_impact_predictor" / "engineered_dataset.csv"
    
    if not data_path.exists():
        print(f"❌ ERROR: Dataset not found at {data_path}")
        print("   Please run preprocessing first:")
        print("   python3 backend/preprocess/prepare_weather_impact_data.py")
        return
    ...
    # ENHANCEMENT: Group K-Fold Cross-Validation (by Track) - only if track available
    if track_train is not None and 'TRACK' in df.columns:
        ...
        # Use 2 splits since we only have 2 tracks in the data
        n_unique_tracks = track_train_val.nunique()
        predictor.evaluate_group_kfold(X_train_val, y_train_val, track_train_val, n_splits=min(n_unique_tracks, 2))

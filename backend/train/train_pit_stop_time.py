"""
Professional Ensemble Pit Stop Time Prediction Model
...
class PitStopTimePredictor:
    """Expert-level ensemble pit stop time prediction model"""
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple:
        ...
        # Define target
        target = 'PIT_STOP_TIME'
        
        # Define features to exclude
        exclude_cols = [
            target, 'PIT_STOP_TIME_BACKUP', 'ELAPSED', 'HOUR', 'FL_time', 'FL_elapsed',
    ...
    def visualize_results(self, X_test, y_test):
        ...
        for idx, (name, metrics) in enumerate(self.metrics.items()):
            ...
            ax.set_xlabel('Actual Pit Stop Time (s)', fontsize=12)
            ax.set_ylabel('Predicted Pit Stop Time (s)', fontsize=12)
    ...
    def save_model(self):
        ...
        # Save ensemble model to backend/model/pit_stop_time_predictor.pkl
        model_path = self.model_dir / 'pit_stop_time_predictor.pkl'
        ...
        metadata_path = self.metadata_dir / 'pit_stop_time_predictor.json'
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("🏎️  GR PitIQ - ENHANCED Pit Stop Time Prediction Model")
    print("=" * 80)
    
    # Get root directory (3 levels up from backend/train/)
    root_dir = Path(__file__).parent.parent.parent
    
    # Load ENHANCED dataset from outputs/pit_stop_time_predictor/
    print("\n📂 Loading dataset...")
    data_path = root_dir / "outputs" / "pit_stop_time_predictor" / "engineered_dataset.csv"
    ...
    predictor = PitStopTimePredictor()

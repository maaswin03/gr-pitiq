"""
Professional Ensemble Optimal Sector Time Prediction Model
LightGBM + XGBoost + CatBoost with Optuna Hyperparameter Tuning
Multi-Output Regression for S1, S2, S3 sector times
"""
...
class OptimalSectorPredictor:
    """Expert-level ensemble optimal sector time prediction model"""
    
    def __init__(self, output_dir: str = None, model_dir: str = None):
        # Get root directory (3 levels up from backend/train/)
        root_dir = Path(__file__).parent.parent.parent
        
        # Set output directory for optimal_sector_predictor
        if output_dir is None:
            self.output_dir = root_dir / "outputs" / "optimal_sector_predictor"
        else:
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple:
        ...
        # Define targets - Multi-output: S1, S2, S3 sector times
        targets = ['S1_SECONDS', 'S2_SECONDS', 'S3_SECONDS']
        
        # Define features to exclude
        exclude_cols = [
            'S1_SECONDS', 'S2_SECONDS', 'S3_SECONDS',  # Targets
            'LAP_TIME_SECONDS', 'LAP_TIME', 'ELAPSED', 'HOUR', 'FL_time', 'FL_elapsed',
        ...
        df_clean = df[(df['S1_SECONDS'].notna()) & 
                      (df['S2_SECONDS'].notna()) & 
                      (df['S3_SECONDS'].notna())].copy()
        
        # Separate features and targets (keep TRACK for stratification)
        X = df_clean[feature_cols].copy()
        y = df_clean[targets].copy()  # Multi-output: S1, S2, S3
        ...
        print(f"   S1: {y['S1_SECONDS'].min():.2f}s - {y['S1_SECONDS'].max():.2f}s (mean: {y['S1_SECONDS'].mean():.2f}s)")
        print(f"   S2: {y['S2_SECONDS'].min():.2f}s - {y['S2_SECONDS'].max():.2f}s (mean: {y['S2_SECONDS'].mean():.2f}s)")
        print(f"   S3: {y['S3_SECONDS'].min():.2f}s - {y['S3_SECONDS'].max():.2f}s (mean: {y['S3_SECONDS'].mean():.2f}s)")
    ...
    def optimize_lightgbm(self, X_train, y_train, X_val, y_val, n_trials=50):
        ...
        def objective(trial):
            ...
            base_model = lgb.LGBMRegressor(**params, n_estimators=500, random_state=42)
    ...
    def optimize_xgboost(self, X_train, y_train, X_val, y_val, n_trials=50):
        ...
        def objective(trial):
            ...
            # Wrap with MultiOutputRegressor for multi-target prediction
            base_model = xgb.XGBRegressor(**params, n_estimators=500, random_state=42)
            model = MultiOutputRegressor(base_model)
            model.fit(X_train, y_train)
    ...
    def optimize_catboost(self, X_train, y_train, X_val, y_val, n_trials=50):
        ...
        def objective(trial):
            ...
            # Wrap with MultiOutputRegressor for multi-target prediction
            base_model = cb.CatBoostRegressor(**params, iterations=500, random_state=42)
            model = MultiOutputRegressor(base_model)
            model.fit(X_train, y_train)
    ...
    def train_ensemble(self, X_train, y_train, X_val, y_val):
        ...
        # Train LightGBM (wrapped for multi-output)
        ...
        lgb_base = lgb.LGBMRegressor(**lgb_params, n_estimators=1000, random_state=42)
        self.models['lightgbm'] = MultiOutputRegressor(lgb_base)
        self.models['lightgbm'].fit(X_train, y_train)
        
        # Store base models for stacking (without MultiOutputRegressor wrapper)
        lgb_base_for_stacking = lgb.LGBMRegressor(**lgb_params, n_estimators=1000, random_state=42)
        
        # Train XGBoost (wrapped for multi-output)
        ...
        xgb_base = xgb.XGBRegressor(**xgb_params, n_estimators=1000, random_state=42)
        self.models['xgboost'] = MultiOutputRegressor(xgb_base)
        self.models['xgboost'].fit(X_train, y_train)
        
        # Store base model for stacking
        xgb_base_for_stacking = xgb.XGBRegressor(**xgb_params, n_estimators=1000, random_state=42)
        
        # Train CatBoost (wrapped for multi-output)
        ...
        cb_base = cb.CatBoostRegressor(**cb_params, iterations=1000, random_state=42)
        self.models['catboost'] = MultiOutputRegressor(cb_base)
        self.models['catboost'].fit(X_train, y_train)
        
        # Store base model for stacking
        cb_base_for_stacking = cb.CatBoostRegressor(**cb_params, iterations=1000, random_state=42)
        
        # Create stacked ensemble using unwrapped base estimators
        print("📦 Creating Stacked Ensemble...")
        estimators = [
            ('lightgbm', lgb_base_for_stacking),
            ('xgboost', xgb_base_for_stacking),
            ('catboost', cb_base_for_stacking)
        ]
        
        # StackingRegressor doesn't natively support multi-output
        # Wrap the entire StackingRegressor with MultiOutputRegressor
        base_stacking = StackingRegressor(
            estimators=estimators,
            final_estimator=Ridge(alpha=1.0),
            cv=5
        )
        self.models['ensemble'] = MultiOutputRegressor(base_stacking)
    ...
    def evaluate_models(self, X_test, y_test, track_test=None, X_val=None, y_val=None):
        ...
        Multi-output evaluation for S1, S2, S3 sector times
        """
        print("\n" + "=" * 80)
        print("📊 Model Evaluation (Multi-Output: S1, S2, S3)")
        print("=" * 80)
        
        results = {}
        sector_names = ['S1_SECONDS', 'S2_SECONDS', 'S3_SECONDS']
        
        for name, model in self.models.items():
            print(f"\n🔍 Evaluating {name.upper()}...")
            
            # Predictions (shape: [n_samples, 3])
            y_pred_test = model.predict(X_test)
            
            # Overall metrics (averaged across all sectors)
            ...
            for i, sector in enumerate(sector_names):
                sector_mae = mean_absolute_error(y_test.iloc[:, i], y_pred_test[:, i])
                sector_rmse = np.sqrt(mean_squared_error(y_test.iloc[:, i], y_pred_test[:, i]))
                sector_r2 = r2_score(y_test.iloc[:, i], y_pred_test[:, i])
                
                results[name]['sector_metrics'][sector] = {
                    'MAE': float(sector_mae),
                    'RMSE': float(sector_rmse),
                    'R²': float(sector_r2)
                }
                
                print(f"      {sector}: MAE={sector_mae:.4f}s, RMSE={sector_rmse:.4f}s, R²={sector_r2:.4f}")
    ...
    def visualize_results(self, X_test, y_test):
        ...
        for idx, (name, metrics) in enumerate(self.metrics.items()):
            ...
            # Flatten errors for multi-output (combines all sectors)
            ...
            ax.set_title(f'{name.upper()} - Error Distribution (Mean: {np.mean(errors_flat):.4f}s)', 
                        fontsize=14, fontweight='bold')
            ax.legend()
            ax.grid(True, alpha=0.3)
    ...
    def save_model(self):
        ...
        # Save ensemble model to backend/model/optimal_sector_predictor.pkl
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("🏎️  GR PitIQ - ENHANCED Optimal Sector Time Prediction Model")
    print("=" * 80)
    
    # Get root directory (3 levels up from backend/train/)
    root_dir = Path(__file__).parent.parent.parent
    
    # Load ENHANCED dataset from outputs/optimal_sector_predictor/
    print("\n📂 Loading dataset...")
    data_path = root_dir / "outputs" / "optimal_sector_predictor" / "engineered_dataset.csv"
    
    if not data_path.exists():
        print(f"❌ ERROR: Dataset not found at {data_path}")
        print("   Please run preprocessing first:")
        print("   python3 backend/preprocess/prepare_optimal_sector_data.py")
        return
    ...
    predictor = OptimalSectorPredictor()
    ...
    # Skip hyperparameter optimization - use default parameters for speed
    print("\n" + "="*80)
    print("⚡ Using default hyperparameters (skipping optimization for speed)")
    print("="*80)
    
    # Set default best params with reduced estimators for faster training
    predictor.best_params_lgb = {
        'n_estimators': 50,
        'max_depth': 6,
        'learning_rate': 0.1,
        'num_leaves': 31,
    ...
    # Skip visualizations and analysis - just save model and metadata
    print("\n" + "="*80)
    print("💾 Saving model and metadata (skipping visualizations)...")
    print("="*80)

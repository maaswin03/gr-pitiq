"""
Professional Ensemble Fuel Consumption Prediction Model
...
class FuelConsumptionPredictor:
    """Expert-level ensemble fuel consumption prediction model"""
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple:
        ...
        exclude_cols = [
            target, 'FUEL_CONSUMPTION_BACKUP', 'ELAPSED', 'HOUR', 'FL_time', 'FL_elapsed',
        ...
        # Check if dataset is large enough for stratification
        n_samples = len(X)
        n_classes = len(track_counts)
        test_size = int(n_samples * 0.15)
        
        # Need at least n_classes samples in test set for stratification
        use_stratification = test_size >= n_classes and n_samples >= 50
        
        if use_stratification:
            print(f"\n✅ Using stratified split (sufficient samples: {n_samples})")
            ...
            print(f"\n⚠️  Using random split (small dataset: {n_samples} samples, {n_classes} tracks)")
            print(f"   Stratification requires at least {n_classes} samples per split")
            # Split data: 70% train, 15% validation, 15% test WITHOUT STRATIFICATION
            X_temp, X_test, y_temp, y_test, track_temp, track_test = train_test_split(
                X, y, track_labels, test_size=0.15, random_state=42
            )
            X_train, X_val, y_train, y_val, track_train, track_val = train_test_split(
                X_temp, y_temp, track_temp, test_size=0.176, random_state=42
            )
    ...
    def visualize_results(self, X_test, y_test):
        ...
        for idx, (name, metrics) in enumerate(self.metrics.items()):
            ...
            ax.set_xlabel('Actual Fuel Consumption (L/lap)', fontsize=12)
            ax.set_ylabel('Predicted Fuel Consumption (L/lap)', fontsize=12)
        ...
        for idx, (name, metrics) in enumerate(self.metrics.items()):
            ...
            ax.set_xlabel('Prediction Error (L/lap)', fontsize=12)
        ...
        ax.set_ylabel('Error (L/lap)', fontsize=12)
    ...
    def save_model(self):
        ...
        # Save ensemble model to backend/model/fuel_consumption_predictor.pkl
        model_path = self.model_dir / 'fuel_consumption_predictor.pkl'
        ...
        metadata_path = self.metadata_dir / 'fuel_consumption_predictor.json'
    ...
    def print_summary(self):
        ...
        print(f"   MAE:  {best_model[1]['MAE']:.4f} L/lap")
        print(f"   RMSE: {best_model[1]['RMSE']:.4f} L/lap")
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("🏎️  GR PitIQ - ENHANCED Fuel Consumption Prediction Model")
    print("=" * 80)
    
    # Get root directory (3 levels up from backend/train/)
    root_dir = Path(__file__).parent.parent.parent
    
    # Load ENHANCED dataset from outputs/fuel_consumption_predictor/
    print("\n📂 Loading dataset...")
    data_path = root_dir / "outputs" / "fuel_consumption_predictor" / "engineered_dataset.csv"
